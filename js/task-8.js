...
const formCarSearch = document.querySelector(".js-form");

const carList = document.querySelector(".js-list");

console.log(formCarSearch);

carList.innerHTML = markUp(cars);

formCarSearch.addEventListener("submit", handlerSearch);
...
function handlerSearch(evn) {
  ...
  const { query, options } = evn.target.elements;
  const inputElem = query.value;
  const select = options.value;

  const resultSearch = cars.filter((car) =>
    car[select].toLowerCase().includes(inputElem.toLowerCase())
  );

  carList.innerHTML = markUp(resultSearch);
}
